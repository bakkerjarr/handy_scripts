...
class TCPACKScan:
    """Use Nmap to port scan a host. A TCP ACK scan is used.
    """
    ...
    def start_scan(self):
        """Start the TCP ACK scan.
        """
        results = {}
        self._file = open(self._RESULT_FILENAME, "a")
        string = ("[?] Starting TCP ACK scan on host {0}".format(
            self._host))
        print(string)
        self._file.write(string + "\n")
...
if __name__ == "__main__":
    ...
    program_desc = "Use Nmap to perform a TCP ACK Scan on a host."
    ...
    # Let's scan!
    tcp_scan = TCPACKScan(args.host_ip4, ports)
    tcp_scan.start_scan()
